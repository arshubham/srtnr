...
fn ui (app: &gtk::Application) {
    ...
    let copy_button =
        gtk::Button::new_from_icon_name("edit-copy", gtk::IconSize::LargeToolbar.into());
    ...
    WidgetExt::set_tooltip_text(&copy_button, "Copy Short Url");
    shorten_url_button.connect_clicked(move |_| {
        ...
        if gtk::ToggleButtonExt::get_active(&goo_gl_radiobutton) {
            ...
            let _googl_short_url = match googl_short_url {
                Ok(googl_short_url) => {
                    label_clone.set_label(&googl_short_url);
                    gtk::WidgetExt::set_sensitive(&copy_button_clone2, true);
                }
                Err(error) => {
                    label_clone.set_label(&std::string::ToString::to_string(&error));
                    gtk::WidgetExt::set_sensitive(&copy_button_clone2, false);
                }
            };
            ...
            label_clone.set_label("Choose a provider");
            gtk::WidgetExt::set_sensitive(&copy_button_clone2, false);
        }
    });
    ...
    gtk::WidgetExt::set_sensitive(&copy_button_clone, false);
